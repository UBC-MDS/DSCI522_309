...
>**Effect of Threshold on Performance**: As evidenced by ROC curve in Fig. 6, an online retailer may want to manually adjust the threshold for determining if an observation is classifed as a `SALE`. The decision for an optimal threshold is highly dependent on whether a retailer cares more about the precision or recall of the model. Our model optimizes the F1 score which is a balance of the precision and recall and can be seen as the point in Figure 7 where all three curves meet. However, an online retailer may prefer to have a more precise model and will thus decrease the recall. Or a retailer may prefer to have a high recall system which then reduces the precision of the model. Figure 7 illustrates how a retailer may want to adjust the the model threshold in order to optimize their business objectives. 
